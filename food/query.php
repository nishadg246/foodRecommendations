...
// curl_setopt($ch, CURLOPT_URL, "40.122.203.139:8000/?word="{$PASTLOC}); //Pass Location of previous sites
    ...
    // curl_close($ch);
$analysis = fopen("output.txt","r") or die("Unable to open file!");
$analysis = json_decode(fread($analysis, filesize("output.txt")));
$counts = $analysis->categoryCounts;
// var_dump($analysis->categoryCounts->mexican);
// $command = system('/var/www/html/food ratings.py $input', $retval);
// echo $command;
$numOfType = array(
        1 => $counts->mexican,
        2 => $counts->chinese,
...
$ignoredPlaces = $analysis->previousRestaurants;
$nameOfType = array(
        1 => 'mexican',
        2 => 'chinese',
        3 => 'indpak',
